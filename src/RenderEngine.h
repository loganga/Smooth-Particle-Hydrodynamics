...
public:
	...
	void display(WorldState & state)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, state.getModel().getPositionBytes(), NULL, GL_STREAM_DRAW);
		glBufferSubData(GL_ARRAY_BUFFER, 0, state.getModel().getPositionBytes(), state.getModel().getPosition());
		glDrawArrays(GL_POINTS, 0, 3 * state.getModel().getParticleCount());
//		checkGLError("draw");
	...
	void reshape(int const & newWidth, int const & newHeight)
	{
		...
		glm::mat4 replaceMe = glm::perspective(PI/3.0f, (float)newWidth/(float)newHeight, 0.1f, 100.0f);
		this->P = replaceMe;
